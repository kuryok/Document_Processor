...
from doc_preprocessor.probe.heuristics import TextBlock, detect_table_heuristic, estimate_columns
...
def analyze_page(page: fitz.Page, config: Config) -> PageMeta:
    ...
    has_large_raster = any(
        int(img[2]) * int(img[3]) > config.thresholds.min_image_area_px2
        for img in image_list
    )

    # 3. Text coverage
    page_area = float(page.rect.width) * float(page.rect.height)
    text_blocks = cast(Sequence[TextBlock], page.get_text("blocks"))
    ...
    text_coverage = covered_area / page_area if page_area > 0 else 0.0
